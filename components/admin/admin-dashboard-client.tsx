    ...
    <div className="flex flex-col gap-5 sm:gap-6">
      <div className="max-w-2xl space-y-1">
        ...
        <p className="text-slate-500 text-sm leading-relaxed">
          This dashboard is front-end only: data lives in the browser until refresh. Each
          card links to a module; connect your API in the client module files when ready.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {modules.map((m) => {
          const Icon = m.icon
          return (
            <Link key={m.href} href={m.href} className="group block h-full min-h-0">
              <Card className="border-slate-200/90 hover:border-[#1b4332]/25 h-full rounded-2xl border bg-white shadow-sm transition-shadow hover:shadow-md">
                <CardHeader className="space-y-3 pb-2">
                  ...
                  <div>
                    <CardTitle className="text-slate-800 text-base font-semibold">{m.label}</CardTitle>
                ...
                <CardContent className="pt-0">
                  {loading ? (
                    ...
                    <p className="text-slate-900 text-3xl font-semibold tabular-nums">{m.count()}</p>
                  )}
      ...
      <Card className="border-slate-200/90 border-dashed bg-white/80 rounded-2xl">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-slate-800 text-base font-semibold">What’s next</CardTitle>
          </div>
          <CardDescription className="text-slate-500 text-sm">
            Add authentication, route guards, and API routes when you are ready. The admin
            modules live under <code className="text-slate-700 text-xs">components/admin</code>{" "}
            and <code className="text-slate-700 text-xs">app/admin</code> for a clean upgrade path.
          </CardDescription>
        </CardHeader>
      </Card>
      <Card className="border-slate-200/90 border-dashed bg-white/80 rounded-2xl">
