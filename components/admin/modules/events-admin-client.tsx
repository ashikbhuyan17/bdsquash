...
import { eventFormSchema, type EventFormSchemaValues } from "@/lib/admin/schemas"
...
function toApiFilters(
  ...
  return {
    ...
    eventStatus: filters.eventStatus === "all" ? undefined : filters.eventStatus,
  }
}
...
export function EventsAdminClient({
  ...
  const loadEvents = useCallback(
    async (nextPage: number, nextPageSize: number, nextFilters: EventsFilterState) => {
      setLoading(true)
      const result = await getEventsAction(toApiFilters(nextPage, nextPageSize, nextFilters))
      setLoading(false)
  ...
  const openCreate = useCallback(() => {
    ...
    form.reset({
      ...defaultForm,
      eventTypeId: eventTypeOptions[0] ? String(eventTypeOptions[0].eventTypeId) : "",
    })
    setSheetOpen(true)
  }, [form, eventTypeOptions])
  ...
  }, [sheetOpen, editing, form, eventTypeOptions])
  ...
  const onSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result = editing
        ? await updateEventAction(editing.eventId, sharedPayload)
      ...
      <AdminListCard>
        <div className="border-slate-100 flex flex-col gap-1 border-b px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4">
          <div className="min-w-0">
            ...
            <p className="text-slate-500 text-sm">
              Create and manage BSRF tournaments with dates, location, and status filters.
            </p>
          </div>
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-slate-600 text-sm font-medium">No events found</p>
                ...
                <TableBody>
                  {rows.map((row) => (
                    <TableRow key={row.eventId}>
                      <TableCell>
                        <ImageThumb src={getEventImageUrl(row.image)} alt={row.name} />
                      </TableCell>
                      <TableCell className="max-w-[14rem]">
                        <p className="text-neutral-900 font-medium break-words">{row.name}</p>
                        <p className="text-neutral-500 text-xs">{row.eventTypeName}</p>
                        <p className="text-neutral-400 text-xs">ID {row.eventId}</p>
                      </TableCell>
                      <TableCell className="hidden text-sm text-neutral-600 lg:table-cell">
                        <p>{formatEventDate(row.startDate)}</p>
                        <p className="text-neutral-400">to {formatEventDate(row.endDate)}</p>
                      </TableCell>
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-slate-500 text-xs">{row.eventTypeName}</p>
                    </div>
                  </div>
                  <p className="text-slate-500 text-xs">
                    {formatEventDate(row.startDate)} – {formatEventDate(row.endDate)}
                  </p>
                  <p className="text-slate-500 line-clamp-2 text-xs">{row.address}</p>
          ...
          <Form {...form}>
            <form onSubmit={onSubmit} className="flex h-full max-h-svh flex-col">
              <SheetHeader className="shrink-0 space-y-1 border-b p-4 sm:p-6">
                ...
                <SheetDescription>
                  Set schedule, location, and image. Images upload as base64 and are served from
                  `/api/event-image/…`.
                </SheetDescription>
              </SheetHeader>
                ...
                <div className="space-y-4 p-4 sm:p-6">
                  {!editing ? (
                    <div className="grid gap-2">
                      ...
                      >
                        <SelectTrigger id="event-type" className="h-10 rounded-lg">
                          <SelectValue placeholder={formPlaceholders.selectEventType} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                      Type: <span className="font-medium">{editing.eventTypeName}</span>
                    </div>
                  )}

                  <div className="grid gap-2">
                    <Label htmlFor="event-name">Name</Label>
                    <Input id="event-name" autoComplete="off" placeholder={formPlaceholders.eventName} {...form.register("name")} />
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="grid gap-2">
                      ...
                      <Input id="event-start" type="datetime-local" {...form.register("startDate")} />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="event-end">End date</Label>
                      <Input id="event-end" type="datetime-local" {...form.register("endDate")} />
                    </div>
                  </div>

                  <div className="grid gap-2">
                    <Label htmlFor="event-description">Description</Label>
                    <Textarea id="event-description" rows={4} placeholder={formPlaceholders.description} {...form.register("description")} />
                  </div>

                  <div className="grid gap-2">
                    <Label htmlFor="event-address">Address</Label>
                    <Input id="event-address" autoComplete="off" placeholder={formPlaceholders.eventAddress} {...form.register("address")} />
                  </div>

                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="grid gap-2">
                      <Label htmlFor="event-latitude">Latitude</Label>
                      <Input id="event-latitude" placeholder={formPlaceholders.latitude} {...form.register("latitude")} />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="event-longitude">Longitude</Label>
                      <Input id="event-longitude" placeholder={formPlaceholders.longitude} {...form.register("longitude")} />
                    </div>
                  </div>
