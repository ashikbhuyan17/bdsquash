...
export function BsrfNewsDetails() {
  const [filter, setFilter] = useState<NewsFilterCategory>('All')

  const articles = useMemo(() => {
    const list =
      filter === 'All'
        ? NEWS_ARTICLES.filter((article) => !article.featured)
        : NEWS_ARTICLES.filter((article) => article.filter === filter)
          ...
          {showFeatured ? (
            <article className="mb-8 grid grid-cols-1 gap-px border border-bsrf-border bg-bsrf-border min-[981px]:mb-12 min-[981px]:grid-cols-[1.1fr_1fr]">
              ...
              <div className="flex flex-col justify-center gap-3 bg-bsrf-card p-5 sm:gap-4 sm:p-6 min-[981px]:p-10">
                <Chip variant="green">{NEWS_FEATURED.cat}</Chip>
                ...
                <ReadLink article={NEWS_FEATURED} />
              </div>
            </article>
          ) : null}
            ...
            {NEWS_FILTER_CATEGORIES.map((category) => (
              <button
                ...
                className={cn(
                  ...
                  filter === category && 'border-bsrf-green bg-bsrf-green text-black'
                )}
