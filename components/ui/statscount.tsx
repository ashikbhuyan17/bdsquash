...
function AnimatedCounter({
	...
	label,
	title = "",
	color = "linear-gradient(90deg, #EA580C 0%, #D946EF 50%, #4F46E5 100%)",
}: {
	value: number;
	suffix?: string;
	duration?: number;
	...
	return (
		...
		>
			...
			<motion.p
				className={cn(
					" dark:text-gray-400 text-sm lg:text-lg font-medium leading-relaxed px-1 sm:px-2 hyphens-auto break-words"
				)}
			...
			<motion.p
				className={cn(
					"text-gray-400 dark:text-gray-400 text-xs sm:text-sm leading-relaxed px-1 sm:px-2 hyphens-auto break-words"
				)}
				style={{ wordBreak: "break-word", overflowWrap: "break-word" }}
				initial={{ opacity: 0 }}
				animate={isInView ? { opacity: 1 } : { opacity: 0 }}
	...
	return (
		...
		>
			...
			<div className={cn("w-full max-w-6xl mx-auto")}>
				<div
					// className={cn(
					//     "flex flex-col sm:flex-row items-stretch justify-between  gap-10 sm:gap-8 w-full min-h-[120px] sm:min-h-[140px] lg:min-h-[160px] py-4 sm:py-6 px-2 sm:px-4 bg-gradient-to-r from-purple-900/20 to-pink-900/20 backdrop-blur-sm border border-purple-500/20 rounded-xl sm:rounded-2xl"
					//   )}
					className={cn(
						"flex flex-col sm:flex-row items-stretch justify-between  gap-5 sm:gap-8 w-full min-h-[120px] sm:min-h-[140px]"
					)}
				>
					{stats.map((stat, index) => (
						...
						>
							...
							{index < stats.length - 1 && showDividers && (
								<motion.div
									className={cn(
										"sm:block hidden absolute -right-1 sm:-right-2 lg:-right-4 top-1/2 transform -translate-y-1/2 h-12 sm:h-16 lg:h-20 w-px bg-gray-200 dark:bg-gray-700"
									)}
